...
async def cmd_poshuk(message: types.Message, state: FSMContext):
    ...
    kb = [
        [KeyboardButton(text="Супи")],[KeyboardButton(text="деСЕРТИ")],[KeyboardButton(text="Гриль")],
        [KeyboardButton(text="Меню")],
    ]
    keyboard = ReplyKeyboardMarkup(keyboard=kb, resize_keyboard=True)
    ...
    @router.message(lambda message: message.text == 'деСЕР', Form.food_class)
...
async def cmd_my_recipes(message: types.Message, state: FSMContext):
    ...
    kb = [
        ...
        [KeyboardButton(text="cmd_all")],[KeyboardButton(text="Меню")],
    ]
    ...
    async def cmd_dodat(message: types.Message, state: FSMContext):
        ...
        async def process_register1(message: types.Message, state: FSMContext):
            ...
            if about1 == 'Супи' or message.text == 'Гриль' or message.text == 'деСЕРТИ':
                ...
                await message.reply(f"Напишіть тепер опис рецепту\nНаприклад: Суп солянка з морквой")
                await state.set_state(Form.description)
            else:
                await message.answer(f"Такого Клаасу рецепту не існує!\n\n{text1}\n\nВводь заново тепер cmd_dodat")
                await state.clear()
    ...
    async def cmd_all(message: types.Message, state: FSMContext):
        ...
        for result in all:
            ...
            await asyncio.sleep(1)
            if result == all[-1]:
                await message.reply(f"Це була вся інформація")
